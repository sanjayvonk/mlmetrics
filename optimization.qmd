...
1. Eliminate $v_t$ and show that $\theta_t$ satisfies the second-order recursion
   ...
   -\delta \theta_{t-1}.
   $$
2. Show that when $\delta=0$, this reduces to ordinary gradient descent on the quadratic objective.
3. Explain why positive momentum can accelerate optimization along directions where the gradient keeps pointing in roughly the same direction, but can also create oscillation when the effective step size is too aggressive.

*Exam level. The exercise turns momentum into a concrete dynamic system and shows how it differs from vanilla gradient descent.*
:::
...
:::{.callout-warning collapse="true"}
## Hint for Part 2
Set $\delta=0$ in the recursion from Part 1.
:::

:::{.callout-warning collapse="true"}
## Hint for Part 3
Compare the recursion with and without the lagged term $-\delta\theta_{t-1}$.
:::
...
**Part 2: Recovering Gradient Descent**

If $\delta=0$, the recursion becomes
$$
\theta_{t+1}=(1-\eta a)\theta_t,
$$
which is exactly the gradient descent recursion for the quadratic objective $L(\theta)=\frac{a}{2}\theta^2$.

**Part 3: Why Momentum Helps and When It Hurts**

The new term $-\delta\theta_{t-1}$ makes the dynamics depend on the previous step as well as the current gradient. If the gradients keep pointing in roughly the same direction over several iterations, this persistence helps the algorithm build up speed and move faster than vanilla gradient descent.

But the same mechanism can also produce overshooting. If $\eta$ and $\delta$ together make the effective step too aggressive, the recursion can oscillate because the algorithm keeps carrying past motion even after the current gradient would call for slowing down. This is why momentum can accelerate convergence in shallow directions but may also create instability if not tuned carefully.
:::
